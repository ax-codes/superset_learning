...
class SupersetAppInitializer:  # pylint: disable=too-many-public-methods
    ...
    def pre_init(self) -> None:
        ...
        """
        wtforms_json.init()  # wtforms_json：校验表单工具库
    ...
    def init_app_in_ctx(self) -> None:
        ...
        """
        self.configure_fab()  # 配置flask_appbuilder
        self.configure_url_map_converters()
        self.configure_data_sources()  # 注册数据源模块
        self.configure_auth_provider()
        self.configure_async_queries()  # 配置异步查询

        # 为管理员提供 Flask APP 的句柄的 Hook
        # 初始化后 可自定义的修改flask
    ...
    def init_app(self) -> None:
        ...
        """
        self.pre_init()  # 预初始化
        self.check_secret_key()  # 检测是否是默认密钥，是的话 提示警告
        # Configuration of logging must be done first to apply the formatter properly
        self.configure_logging()  # 配置日志
        # Configuration of feature_flags must be done first to allow init features
        # conditionally
        self.configure_feature_flags()  # 配置启用功能标识
        self.configure_db_encrypt()  # 加密数据库指定数据
        self.setup_db()
        self.configure_celery()  # 配置调度任务
        self.enable_profiling()  # 打开性能分析
        self.setup_event_logger()  # 事件记录器 数据库事件记录器
        self.setup_bundle_manifest()  # 注册前端生成的bundle
        self.register_blueprints()  # 注册蓝图
        self.configure_wtf()  # 配置wtf WTForms 是一个用于 Python Web 开发的灵活的表单验证和渲染库
        self.configure_middlewares()  # 配置中间件
        self.configure_cache()  # 配置缓存
    ...
    def setup_event_logger(self) -> None:
        _event_logger["event_logger"] = get_event_logger_from_cfg_value(
            self.superset_app.config.get("EVENT_LOGGER", DBEventLogger())  # 数据库事件记录器
        )
    ...
    def setup_db(self) -> None:
        ...
        with self.superset_app.app_context():
            pessimistic_connection_handling(db.engine)  # orm监听
